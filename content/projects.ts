...
export const projects: Project[] = [
  {
    ...
    approach:
      "Instead of a fake sync, I built forward-direction enrichment. When a booking fires, the bridge looks the agent up in GoHighLevel and creates or updates the contact, and a one-time historical backfill runs through the platform's CSV export path. The client got a complete, continuously updated CRM and zero pretend two-way sync.\n\nThe integration runs on self-hosted n8n on a hardened Linux VPS. Three production event workflows, booking created, report delivered, and payment completed, route every inspection across a 50-service catalog. Every inbound event dedupes against a composite-keyed Postgres ledger, so vendor retry storms cannot double-write the CRM. Phone-number collisions get caught, retried without the colliding field, and tagged for human review instead of failing silently. An alerting workflow polls hourly and emails the moment any event fails.\n\nThe attribution layer ties a marketing touch to the booking it eventually drives. Per-event QR codes feed a tagged capture form and a structured nurture sequence, and the booking webhook traces that touch back to the booking months later.",
    stackDetailed:
    ...
    result:
      "Live in production since May 2026, carrying the business's real booking, report-delivery, and payment events, its real revenue data, not test traffic. A point-in-time read from the live n8n dashboard showed 318 production executions, 0 failed, a 0% failure rate, and a 0.7-second average run time. The residential pipeline alone holds 5,840 opportunities.\n\nThe historical migration moved roughly 8,000 contacts and 6,500-plus opportunities with clean deduplication and zero lost relationship history. The engagement is ongoing. The lifecycle-automation milestone is live, and a custom team-operations dashboard is live behind auth on the client's own subdomain.",
    stats: [
  ...
  },
  {
    ...
    sections: [
      {
        heading: "Context",
        body: "My client has spent 25 years in the transportation industry and he buys data for a living. He has been burned by vendors selling stale lists, so he keeps a master answer key, contacts he already knows are correct, and grades every vendor's sample against it. He told me straight up the test exists to catch fakers.\n\nHe did not hire me off a pitch. He paid me $300 for a small calibration batch in his exact template and graded it.",
      },
      {
        heading: "The pilot",
        body: "I scored 8 out of 10 on his grading. His notes on the misses became the spec for the real build. Three things happened during that pilot that mattered more than the score. My freshness check flagged that one of his own confirmed contacts had quietly changed jobs. One miss was my own too-strict filter benching a valid contact, and I said so instead of arguing the edge case. He had assumed I was scraping LinkedIn. I was not, and I corrected him even though the assumption was working in my favor.",
      },
      {
        heading: "The build",
        body: "He funded a $4,500 owned-code engine that handles sourcing, enrichment, verification, and scoring, delivered in his template. He owns the code outright. I charge for the build and optional maintenance. His raw data costs pass through at cost, where he can see them, never blended into my fee.",
      },
    ],
    stackDetailed:
  ...
  },
  {
    ...
    sections: [
      {
        heading: "Context",
        body: "A home inspection company owner had already tried to build this himself. He put hundreds of hours into ChatGPT and Gemini versions, and they kept drifting. The rules got lost somewhere in one long chat and the model started improvising. That is the usual ending for AI a business bolts on itself, and it is why he brought it to me instead of trying a third chatbot.",
      },
      {
        heading: "The build",
        body: "I built the v1 engine. It reads his real inspection reports end to end and produces the review in his own scorecard format, the same layout he had been filling in by hand at night. His own standards are wired in as grounding, so it flags things the way he does, not the way a generic chatbot would.",
      },
      {
        heading: "Why it holds",
        body: "The model was never his problem. In my build the rules live outside the model. Each check hands the AI only the slice of his standards it needs for that one question, so it cannot slowly lose the plot the way a chat window does. A prompt is not a system. The machinery around the model is what makes AI dependable inside a real business.",
      },
      {
        heading: "The number I will not publish yet",
        body: "I could quote an accuracy number today and it would look great. It would also be close to meaningless, because the reports I have are the versions he already corrected, and testing on those flatters the engine. The real catch rate gets measured against reports the way his inspectors first submitted them, and until then I am not quoting one. When a vendor hands you a suspiciously clean accuracy number, ask what they tested it on.",
      },
    ],
    stackDetailed:
